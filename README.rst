...
- Bump the release date in ``CITATION.cff`` to the current date
- Bump the version in ``pyproject.toml`` and ``CITATION.cff`` to a stable one, e.g. ``0.6.0``, commit and push, wait until the build completed. An example commit message would be ``No issue. Release 0.6.0``
